...
class TaoBao:
    def __init__(self, str_searchContent, num_pageSize, num_page, appKey, threads_num_get_pages,
                 switch_save, proxies, threads_num_get_comments):
    ...
    def second_requests(self):
        ...
        pageSize = str(self.num_pageSize)  # 每页结果属
        ...
        try:
            with requests.get(url, headers=headers) as res:
                html = res.text
                res_str = html.split(
                    '"resultList":')[-1].split('}},"ret":')[0].replace('true', '"true"').replace('false', '"false"')
    ...
    def get_comments(self, itemId):
        ...
        url = f'https://rate.tmall.com/list_detail_rate.htm?itemId={itemId}&spuId=1834646277&sellerId=2206843128012&order=3&currentPage=2&append=0&content=1&tagId=&posi=&picture=&groupId=&ua=098%23E1hvgQv7vOpvUvCkvvvvvjiWP2My6jiUR2qOzjthPmPWlj38RssUzjDnP2SO6jlERvhvCvvvphm%2BvpvBCU2ktepvvCKAEBYJ6ahtuk4%2BvpvEvUmIdvwvvvuVdvhvmpmv%2FPA5vvmUMuQCvvyvvOeU%2BvvvCwIgvpvhvvCvpv9CvhACUvGPjNoAdBeXjL4xfBuKHdBYLnsvV1OqrADn9Wma%2BfmtEpch1C6t%2BFIlBqev%2Bu0O5C60dbm65iI4wh7Q0fJ6W3CQog0HKfUpejEUkvhvC9hvpyP9s89Cvv9vvhh6A77bef9Cvm9vvhCvvvvvvvvvByOvvvHivvCVB9vv9LvvvhXVvvmCjvvvByOvvUhwvvhvC9mvphvvvvgCvvpvvPMMRvhvCvvvphmevpvhvvCCBUOCvvpv9hCv9vhvj3gGhDi%2FzYswzjA875R2uU9E9fq2gFSEAUHU1%2BDsvQOCvvNwzHi4zAmlRvhvCvvvphm%2BvpvEvvhb2MQvvELAdvhvhpov%2BbpfvvmOYdO3ob5aRfvgdvhvmpvCcHYJvvv9RgvCvCLwMHnaCaMwznQhhxS5CrApzVD44v%3D%3D&needFold=0&_ksTS={tmp[0]}_{tmp[1]}&callback=jsonp{int(tmp[1]) + 1}'
        referer = f'https://detail.tmall.com/item.htm?id={itemId}'
        headers = {'cookie': self.cookie, 'referer': referer, }
        with requests.get(url, headers=headers) as res:
            comments = res.text.split('"rateList":')[1].split(',"searchinfo"')[0].replace('true', '"true"').replace(
...
if __name__ == "__main__":
    ...
    threads_num_get_comments = 3  # 抓取评论页的线程数,当为 0 时,不抓取详情页面(评论)； -->待开发完善
    ...
    TaoBao(str_searchContent, num_pageSize, num_page, appKey,
           threads_num_get_pages, switch_save, proxies, threads_num_get_comments)
